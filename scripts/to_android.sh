#!/usr/bin/env bash
# Transfers media files or pushes notifications to an Android device via pluggable backends (default: KDE Connect).
# Designed to grow beyond simple "sharing" so other delivery mechanisms can plug in over time.
...
Options:
  ...
  -b, --backend B  Android transfer backend to use (default: kdeconnect)
  -a, --action A   action to run: send-file (default) or notify
  --message TEXT   notification body for --action notify (defaults to positional args)
...
SUPPORTED_ACTIONS=(send-file notify)

action_supported() {
  local candidate="$1"
  local action_name
  for action_name in "${SUPPORTED_ACTIONS[@]}"; do
    if [[ $candidate == "$action_name" ]]; then
      return 0
    fi
  done
  return 1
...
backend_kdeconnect_share() {
  ...
  for file in "${files[@]}"; do
    print_debug "Sending $file via KDE Connect"
    output=$(kdeconnect-cli --device "$kdeconnect_selected_device_id" --share "$file" 2>&1)
    status=$?
    if (( status != 0 )); then
      print_error "Failed to send $file via KDE Connect."
...
backend_kdeconnect_notify() {
  local message="$1"
  if [[ -z $message ]]; then
    message="$notification_message"
  fi
  if [[ -z $message ]]; then
    print_error 'Notification message cannot be empty.'
    exit 2
  fi
  if [[ -z $kdeconnect_selected_device_id ]]; then
    print_error 'Internal error: no KDE Connect device selected.'
  ...
  print_debug "Sending notification via KDE Connect"
  local output status
  output=$(kdeconnect-cli --device "$kdeconnect_selected_device_id" --ping-msg "$message" 2>&1)
  status=$?
  if (( status != 0 )); then
    print_error 'Failed to send notification via KDE Connect.'
    print_error "Command output: $output"
    exit 4
  elif (( verbose )); then
    print_debug "kdeconnect-cli output: $output"
...
backend_share() {
  ...
  case "$backend_name" in
    kdeconnect)
      ...
      ;;
    *)
      print_error "Transfer backend not implemented: $backend_name"
      exit 2
      ;;
  esac
}
...
while (( $# )); do
  case "${1:-}" in
    ...
    -a|--action)
      shift
      [[ -n ${1:-} ]] || { print_error 'Missing value for --action'; exit 2; }
      action=${1,,}
      shift
      ;;
    --message)
      shift
      [[ -n ${1:-} ]] || { print_error 'Missing value for --message'; exit 2; }
...
if ! action_supported "$action"; then
  print_error "Unsupported action: $action"
  print_supported_actions >&2
  exit 2
fi
...
case "$action" in
  ...
  notify)
    if [[ -n $notification_message ]]; then
      if (( $# > 0 )); then
        print_error 'Unexpected positional arguments for notify when --message is used.'
        exit 2
      fi
    else
      if (( $# == 0 )); then
        print_error 'Missing notification message. Provide --message or positional text.'
        exit 2
      fi
      notification_message="$*"
    fi
...
if (( verbose )); then
  print_debug "Transfer backend: $backend"
  print_debug "Action: $action"
fi

if [[ $action == "send-file" ]]; then
  for file in "${files[@]}"; do
    if [[ ! -f $file ]]; then
  ...
  if (( verbose )); then
    print_debug "Notification message: $notification_message"
  fi
fi

declare -a action_payload=()
if [[ $action == "send-file" ]]; then
  action_payload=("${abs_files[@]}")
else
  action_payload=("$notification_message")
fi
...
if (( ${#ipv4_clients[@]} == 0 )); then
  if [[ $backend == "kdeconnect" ]]; then
    ...
    backend_share "$backend" "${action_payload[@]}"
    exit 0
  fi
