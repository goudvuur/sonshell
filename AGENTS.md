...
- Auto-sync default: even with `--sync-dir`, downloads stay off after boot until the user runs `sync on` (manual `sync` / `sync all` still work).
...
- Command surface (see README): REPL supports `status`, `shoot`, `focus`, `sync` variants, `exposure` subcommands, `monitor start/stop`, `record start/stop`, `button ...`, `power off`, `quit`. Auto-sync requires `--sync-dir`; fingerprint cache under `~/.cache/sonshell/`.
